eva.controller('woo', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.list = function () {
        ...
        $http.get(`${URL}/api/api/common?db=voice`).then(function successCallback(response) {
    ...
    $scope.create = function () {
        $http.post(`${URL}/api/api/common?db=woo`, { name: $scope.name, command: [], order: $scope.order.split(',') }).then(function successCallback(response) {
            $scope.list();
    ...
    $scope.updatesend = function (flag) {
        ...
        $http.put(`${URL}/api/api/common/` + $scope.tempwoo._id + '?db=woo', json).then(function successCallback(response) {
    ...
    $scope.delete = function () {
        if (confirm(locale().COMMON.DELETE)) {
            $http.delete(`${URL}/api/api/common/` + $scope.tempwoo._id + '?db=woo').then(function successCallback(response) {
                $scope.list();
            }, function errorCallback(response) {
                notify(locale().WOO.NOTIFY.ERROR, 'danger');
            });
