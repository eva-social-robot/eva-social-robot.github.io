...
function table(obj) {
    ...
    $('#listadoScriptdata').bootstrapTable({
        url: `${URL}/api/scriptdata/${obj.value}`,
        pagination: true,
        search: true,
        searchTimeOut: 1000,
        locale: locale().CODE,
...
function setForUpdateScriptdata(id) {
    getData(`${URL}/api/scriptdata/${id}`).then(edit => {
...
function deleteScriptdata(id) {
    deleteData(`${URL}/api/scriptdata/${id}`)
        .then((data) => {
